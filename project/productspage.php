...
<html>
	<head>
		<title><?php echo $_GET['title'];?> on Ukart</title>
	...
	<body>
		...
		<div class="loginModal" id="loginModal"<?php if($aregistered==1||$loginerr==1){echo " style=\"display:block\"";}?>>
			<div class="loginModalContent">
				...
				<form class="loginForm" name="loginForm" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]."?title=".$_GET['title']);?>">
		...
		<div class="signupModal" id="signupModal"<?php if($passworderr==1){echo " style=\"display:block\"";}?>>
			<div class="signupModalContent">
				...
				<form class="signupForm" name="signupForm" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]."?title=".$_GET['title']);?>">
							...
							<ul>
								...
								<li id="usernamedropdown" <?php if(isset($_SESSION["loginname"])){echo"style=\"display:block\">".$_SESSION["loginname"];}?><i class="downarrow"></i>
									<div class="dropdowncontent">
										...
										<a href="productspage.php?state=loggedout&title=<?php echo $_GET['title'];?>">Log out</a>
									</div>
								</li>
							</ul>
						</span>
		...
		<nav class="flex_container" id="navbar">
			...
			<div id="Offers">
				<a href="productspage.php?title=Offers+Zone">Offers Zone</a>
			</div>
		</nav>
		...
		<?php
			if($_GET['title']=='Television')
			{
				$sql=array("Xiaomi","Vu","LG","Samsung","Sony");
				for($i=0;$i<5;$i++)
				{	
					echo"
					<div class=\"flex_container1\">
						...
						<div class=\"lowercont\">";
							$result = $conn->query("select * from proinfo where category like '%TV ".$sql[$i]."%'");
							if($result->num_rows>0)
							{
